...
namespace MVC5Course.Controllers
{
    public class ClientController : Controller
    {
        ...
        // GET: Clients
        public ActionResult Index(int? CreditRationFilter = -1 , string lastNameFilter = "", int pageNo = 1)
        {
            var ratings = (from p in db.Client
                           select p.CreditRating)
                           .Distinct()
                           .OrderBy(p => p).ToList();
            ViewBag.CreditRationFilter = new SelectList(ratings);
            ...
            if (CreditRationFilter >= 0)
                client = client.Where(x => x.CreditRating == CreditRationFilter);
